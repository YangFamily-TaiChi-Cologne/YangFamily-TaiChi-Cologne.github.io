...
- Local files are accessible; Joel (Discord user) does not have direct file access

## Workflow for Discord requests (from Joel)

Joel will send requests via Discord. When a message arrives from him:
...
4. **Notify Joel via Discord** — ping him when the deployment is triggered (push done), or immediately if you have questions or the request cannot be completed
